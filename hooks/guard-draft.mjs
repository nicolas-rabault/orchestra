...
//
// The drafts directory is gitignored by the file `orchestra init` writes, so an ordinary `git add`
// of a draft is already refused by git itself, and the only way past that refusal is `-f` — but a
// project that has not run `init` yet, or has since edited its own `.gitignore`, has no such
// floor. So, unlike the guard this is spun off from in the source project (planetCraft's
// `guard-local-roadmap.mjs`, which only had the gitignore-bypass case to cover and so only ever
// looked for `-f`), this hook does not require `-f` at all: it refuses NAMING a path under
// `cfg.roadmaps.drafts` on a `git add`, whether or not `-f` is present — see
// `lib/guards/draft.mjs`'s header for the decision itself.
//
// What this cannot catch: `git add -A` (or a bare `git add .`) sweeps a draft into the index
// without ever naming it, and only the gitignore stops that case.
//
