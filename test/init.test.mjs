...
test('initProject online: writes .orchestra/.gitignore with exactly the paths the plugin creates, and nothing else', () => {
  ...
  assert.deepEqual(lines, [
    ...
    'worktrees/', 'images/', 'gate/', 'drive/', 'tick.sh', '*.log', '*.err', '*.tmp',
  ]);
...
test('initProject online: the .gitignore hides the pull-request ledger and the direction memory', () => {
  ...
  // repo, a real `init`, the real files, and `git status` must report nothing. ONLINE mode, because
  // that is the mode this file is written in: offline writes no `.gitignore` at all and excludes
  // the whole of `.orchestra/` in the clone instead, which covers both paths a fortiori.
  const r = repo();
  rmSync(join(r.root, '.orchestra'), { recursive: true, force: true });
  initProject(r.root, { mode: 'online' });
  r.git('add', '.orchestra/config.json', '.orchestra/.gitignore', 'CLAUDE.md');
