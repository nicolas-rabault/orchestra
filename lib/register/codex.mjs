...
export function codexOutbound(row, now = Date.now(), { maxAutoResumes = MAX_AUTO_RESUMES } = {}) {
  if (!relayOwed(row) && resumeLimit(row, maxAutoResumes)) throw new Error('resume budget reached; inspect the report and narrow the task before renewing');
...
export function attachCodex(row, threadId, hostId = 'local') {
  ...
  return { ...rest, previousSession: row.session ? { runtime: row.runtime ?? 'claude', session: row.session, sessionName: row.sessionName ?? null, model: row.model ?? null, thinking: row.thinking ?? null, hostId: row.hostId ?? null } : row.previousSession ?? null,
    runtime: 'codex', session: threadId, hostId, sessionName: null, model: null, thinking: null };
}
export function recordDispatch(row, data, now = Date.now(), cfg = {}) {
  ...
  const current = codexOutbound(row, Date.parse(data.preparedAt), cfg).baseline;
  ...
  return { ...row, autoResumes: (row.autoResumes ?? 0) + (relay ? 0 : 1), dispatch: { ...data }, observation: { threadId: row.session, hostId: row.hostId ?? 'local', status: 'running', observedAt: data.acceptedAt } };
}
