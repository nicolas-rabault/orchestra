...
// no opinion about WHO is being told. Two entry points share it: the `orchestra inbox` subcommand,
// and the `orchestra-inbox` hook (`hooks/orchestra-inbox.mjs`) that injects it into a live session.
// Only the hook has a session gate; the rendering below is identical for both by construction, so
// there is no second copy of the branches here to rot out of sync.
//
...
import { pendingId, runAskId } from './pending.mjs';
...
function about(e, rows, runAsks) {
  // No task and an item id: this answers a question about the RUN — the one ask that hangs on no
  // row, and the one the protocol requires at the end of a run, when no row is left to hang on.
  // Without this branch it reached the conductor as "a remark for you", losing both which question
  // it answered and the words of that question.
  if (!e.task && e.pending) {
    const item = (runAsks ?? []).find((a) => runAskId(a) === e.pending);
    if (!item) return [`item ${e.pending} — a run-level question that is NO LONGER in runAsks[]; establish what was asked before acting on this`];
    return [`the run${item.roadmap ? ` ${item.roadmap}` : ''} · item ${e.pending} · ${item.kind ?? 'question'}`,
      `the question: "${oneLine(item.ask, ONE_LINE_CAP)}"`,
      'about the run itself and not about any row — there is no session to forward it to: act on it, and move the item into `runAnswered[]`'];
  }
...
export function relay(repo, now = Date.now()) {
  ...
  const lines = shown.map((e) => `- [${e.ts}] ${about(e, rows, state.runAsks).join('\n  ')}\n  the user's answer: "${e.answer}"`);
  ...
  return `<orchestra-monitor-inbox count="${shown.length}">\n${lines.join('\n')}\n</orchestra-monitor-inbox>\n\n`
    ...
    + 'named above (with whatever context that worker needs), retire the item it answers — a row\'s '
    + '`pending[]` item into that row\'s `answered[]`, a run-level ask into `runAnswered[]`, keeping '
    + 'its `askedAt` and gaining an `answeredAt` — and journal an `answer` line. Then set '
    + '`conductor.inboxSeen` to the '
