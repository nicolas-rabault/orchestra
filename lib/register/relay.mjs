...
// and (from P4) the hook that injects it into a live session. Only the hook has a session gate; the
// rendering below is identical for both by construction — both call this same function, so there is
// no second copy of the branches here to rot out of sync unnoticed.
//
...
import { readJsonl, unconsumed, inboxPath } from './inbox.mjs';
...
const oneLine = (s) => {
  ...
  return flat.length > ONE_LINE_CAP ? `${flat.slice(0, ONE_LINE_CAP)}…` : flat;
};
...
function about(e, rows) {
  ...
  return [`${e.task} · item ${e.pending} · ${item.kind ?? 'question'}`, `the question: "${oneLine(item.ask)}"`, addressee(row, 'asked by')];
}
...
export function relay(repo, now = Date.now()) {
  ...
  const shown = fresh.slice(0, RELAY_CAP);
