...
export function announcement(obl, memory, now = Date.now()) {
  ...
  const key = [...due.map((e) => `i:${e.id}`), ...obl.undelivered.map((e) => `u:${e.id}`), ...(obl.refresh ?? []).map((e) => `r:${e.id}`), ...(obl.limited ?? []).map((e) => `l:${e.id}`)].sort().join(' ');
  if (!key) { memory.key = null; return null; }
  if (key === memory.key && (!due.length && !obl.undelivered.length && !obl.refresh?.length || now - memory.at < REMIND_MS)) return null;
  memory.key = key;
  memory.at = now;
  const parts = [];
  ...
  if (obl.limited?.length) parts.push(`${obl.limited.length} worker(s) reached their resume budget (${names(obl.limited)}); inspect and narrow before renewing`);
  if (obl.refresh?.length) parts.push(`${obl.refresh.length} native task snapshot(s) need refresh (${names(obl.refresh)}); use wait_threads then codex observe`);
...
export function round(root, cfg, memory, { now = Date.now(), liveness = null, emit = console.log } = {}) {
  ...
  const line = announcement(obligations(tasks, liveness ?? gatherLiveness(root, tasks), { now, maxAutoResumes: cfg.maxAutoResumes }), memory, now);
  if (!line) return 0;
  emit(line);
  return 1;
}
