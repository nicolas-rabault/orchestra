...
// matters, and which P4's monitoring page will test for existence the same way. What is new is
// `holder.json` INSIDE it, so the lock can say who has it and whether that somebody is still alive.
//
...
//   - `conductor`: an interactive session. It has NO pid a shell could name — `node lock.mjs
//     acquire` exits immediately, so recording its own pid would mark the lock dead on the spot.
...
// tick measured on this machine ran a full suite at ~4.5 minutes — and short enough that a lock
// orphaned by a kill -9 does not freeze the heartbeat for a working day.
...
// between mkdir and the holder write. Only age can judge it.
