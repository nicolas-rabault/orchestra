...
import { resumeLimit, MAX_AUTO_RESUMES } from './workerPolicy.mjs';
...
export function running(row, liveness, now) {
...
export function obligations(tasks, liveness, { now = Date.now(), maxAutoResumes = MAX_AUTO_RESUMES } = {}) {
  const limited = [];
  const idle = [];
  const undelivered = [];
  const driving = [];
  ...
  for (const t of tasks) {
    ...
    if (!t.session && (t.status !== 'todo' || t.autoResumes !== undefined) && !owed && resumeLimit(t, maxAutoResumes)) { limited.push(base); continue; }
    if (!t.session) {
    ...
    if ((t.runtime ?? 'claude') !== 'claude') {
      const status = t.runtime === 'codex' ? codexStatus(t, now) : 'unknown';
      if (status === 'running') driving.push({ ...base, why: 'Codex native task running', turn: null, relayInFlight: owed });
      else if (status === 'unknown') refresh.push({ ...base, runtime: t.runtime, why: 'refresh native task snapshot; never relaunch from stale observation' });
      else if (owed) undelivered.push({ ...base, writtenAt: t.relay.writtenAt ?? null, ageMin: ageMin(t.relay.writtenAt, now), registered: null, turn: null });
      else if (status === 'completed' && !isAskedOf(t) && t.status !== 'review') {
        if (resumeLimit(t, maxAutoResumes)) limited.push(base);
        else idle.push({ ...base, registered: null, turn: null });
      }
      continue;
    }
    ...
    if (resumeLimit(t, maxAutoResumes)) limited.push(base);
    else idle.push({ ...base, registered, turn });
  }
  return { idle, undelivered, driving, relaunch, ...(limited.length ? { limited } : {}), ...(refresh.length ? { refresh } : {}) };
}
...
export const nudgeFor = (row) => (relayOwed(row)
  ...
  : `${CONTINUE[0].toUpperCase()}${CONTINUE.slice(1)}`) + `\n${progressInstructions(row.id)}\nBefore returning your final report or question, call orchestra notify ${row.id}. This is only a wake signal; still write your final report. If unavailable, do not retry in a loop.`;
