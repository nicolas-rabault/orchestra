...
export function obligations(tasks, liveness, { now = Date.now() } = {}) {
  ...
  for (const t of tasks) {
    ...
    if ((t.runtime ?? 'claude') !== 'claude') {
      ...
      else if (status === 'completed' && !isAskedOf(t) && t.status !== 'review') idle.push({ ...base, registered: null, turn: null });
      continue;
    }
  ...
  return { idle, undelivered, driving, relaunch, ...(refresh.length ? { refresh } : {}) };
}
