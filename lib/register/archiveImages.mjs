...
// pictures that prose points at, and those are the weight: measured 2026-09-02, in planetCraft,
// `.claude/orchestra/` held **70.2 MB in 199 files**, of which 68 MB was 114 screenshots and
// boards — 33 MB for one A/B board of the render gate (`x4-gate-board/`, shot 2026-08-19), 16 MB
// for `shots/`, and 19 MB of loose comparison PNGs at the top level. Every one of them belonged
// to a run that had landed weeks earlier, and every conclusion drawn from them is written down
// somewhere a picture is not needed to read it — `docs/superpowers/results/` for the results, the
// row's own note for the rest.
//
...
// are exactly the three surfaces the monitoring page (`monitor/model.mjs`) already scans, which is
// why the citation scanner lives in its own module (`images.mjs`) rather than being written again
// here:
//
...
// exists because the prose pass REWRITES `state.json` under a conductor holding it in memory for
...
// of montages with relative hrefs — is how this repository, in planetCraft, photographs a whole
// render-gate run, and the single heaviest file in the directory is one
// (`shots/gt9-viewmodel-ab.html`, 10.2 MB).
...
const PHOTO_EXTENSIONS = [...IMAGE_EXTENSIONS, 'html'];
