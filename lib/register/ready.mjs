...
export function computeReadySet(tasks, git) {
  ...
  // bare id, and the board (tools/roadmap/board.mjs) already resolves every dep to a qualified
...
// A relay WRITTEN into a row is not a relay DELIVERED to its worker, and nothing used to hold the
// difference. Measured 2026-08-13/14: a blocking defect written onto dev-loop/S3 at 18:35 reached
// its worker at 04:35 the next morning — eight hours and sixteen minutes, the most expensive single
// stall of that roadmap. The row was `review`, which the liveness step reads as "waiting on the
// user" and therefore as "nothing to do"; seven consecutive ticks looked at it and wrote "nothing
// moved, and that is normal". So status is deliberately NOT part of this filter: a row can owe the
// user an answer and owe its worker a message at the same time, and S3 owed both.
//
// Terminal rows are excluded for the same reason `reconcileTasks` never resurrects them — a landed
// row's undelivered relay is history, not work.
export function undeliveredRelays(tasks, { now = Date.now() } = {}) {
  return tasks
    .filter((t) => !TERMINAL.has(t.status) && t.relay?.text && !t.relay.deliveredAt)
    .map((t) => {
      const at = Date.parse(t.relay.writtenAt ?? '');
      return {
        id: t.id,
        status: t.status,
        writtenAt: t.relay.writtenAt ?? null,
        // A relay with no readable timestamp is still undelivered — report it with no age rather
        // than with NaN, which prints as "NaN min ago" and reads as a bug in the report.
        ageMin: Number.isFinite(at) ? Math.round((now - at) / 60_000) : null,
      };
    });
}
...
// specific checkout.
export function gatherGit(root) {
  const env = gitEnv();
  const sh = (cmd) => execSync(cmd, { cwd: root, encoding: 'utf8', env }).trim();
  const branches = sh("git for-each-ref --format='%(refname:short)' refs/heads")
    .split('\n').filter((b) => b && b !== 'main');
  const mainSubjects = sh('git log --format=%s -200 main').split('\n').filter(Boolean);
