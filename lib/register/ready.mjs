...
import { resumeLimit, workerSelection } from './workerPolicy.mjs';
import { relayOwed } from './drive.mjs';
import { workerRuntime } from './runtime.mjs';
...
export function computeReadySet(tasks, git) {
  ...
  ready.sort((a, b) => (a.order ?? Infinity) - (b.order ?? Infinity) || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
...
export function planLaunches(ready, inFlightCount, width, runtimeOptions = {}) {
  const ordered = [
    ...ready.filter((t) => t.mine !== false),
    ...ready.filter((t) => t.mine === false),
  ];
  return ordered.filter(t => t.autoResumes === undefined || !resumeLimit(t, runtimeOptions.maxAutoResumes) || relayOwed(t))
    .slice(0, Math.max(0, width - inFlightCount))
    .map((t) => {
      const runtime = workerRuntime(t, runtimeOptions);
      return { ...t, runtime, ...workerSelection(runtimeOptions, t, runtime) };
    });
}
...
// WARM SESSIONS BEFORE COLD ONES, as a rule of the tool rather than a habit of the conductor.
//
// A new worker pays for its entire brief before it produces a line; a session already open and
// already on the problem pays for nothing. Measured 2026-09-08 in duckJam, and written into the
// journal that morning as a `ruling` the conductor had to invent by hand because nothing here said
// it: six lines launched at 06:45 were every one of them refused AFTER reading their brief, while
// `cd8-exec` — one already-warm, already-targeted session — produced nine commits in the same
// window. Its twin measurement says the same thing from the other side: at `ready --width 40` not
// one extra line was launchable, every one blocked on a dependency, and harvesting three resting
// workers unblocked seven. Width was never this run's constraint; the resting fleet was.
//
// So while a turn is owed, the launch plan stands down and NAMES what is holding it. Nothing here
// can wedge: every row that holds the plan back has one command that clears it (`orchestra drive`,
// or a relaunch for a row whose session or worktree is gone), and a relaunch of an existing claimed
// row is not a `planLaunches` launch at all — it is outside this decision, so the way out is never
// the thing being held.
//
// The order this enforces is also the order the tick already runs in: obligations at step 3, launches
// at step 8. What was missing is that the PLAN was computed at step 1, against a fleet the tick had
// not yet harvested, and then followed at step 8 as though it still described one.
export function launchHold(launches, { idle = [], undelivered = [] } = {}) {
  const owed = [...undelivered, ...idle].map((e) => e.id);
  if (!owed.length || !launches.length) return null;
  return { owed, deferred: launches.map((t) => t.id) };
}
