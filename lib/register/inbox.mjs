...
// Re-exported, not re-implemented: the reader moved to ../jsonl.mjs when the merge gate's attempt
// ledger became the third file with the identical shape, and every caller that already imports it
// from this module — the page's sources, the archiver, the tick gate, the watch, the relay — reads
// exactly what it read before.
export { readJsonl } from '../jsonl.mjs';
...
//     indistinguishable from the stamp, and `after()` below would silently drop it — a caller
...
export function openPendingIds(state) {
  ...
  // A question about the run itself is open the same way, and it is open in the one state where no
  // row can hold it — every row terminal. Left out of this set, an answer to it would lose the
  // grace half of `unconsumed` below: the half that carries a reply past a session which died
  // before relaying it.
  for (const a of state.runAsks ?? []) ids.add(runAskId(a));
  return ids;
}
...
// pending id is a hash of row/kind/ask — a hands-on gate asked twice in the same words carries the
