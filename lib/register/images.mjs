...
export function imagesIn(text, findImage, branch = null) {
  // Not a hypothetical guard: `buildModel` defaults `findImage` to `null` (a resolver is not
  // always available — a reader with no filesystem under it cannot honestly report a file as
  // present OR as missing), so this function is handed a non-function for real, not only in a test.
