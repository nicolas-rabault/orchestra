import { workerRole } from './workerPolicy.mjs';
  ...
  for (const dir of [cfg.docs.specs, cfg.docs.plans]) {
    ...
    try {
      for (const f of readdirSync(abs).sort()) {
        if (new RegExp(`(^|[^a-z0-9])${needle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}([^a-z0-9]|$)`).test(f.toLowerCase())) hits.push(`${join(dir, f)} — ${sizeOf(cfg.root, join(dir, f))}`);
      }
...
function hardRules(cfg) {
  const tests = cfg.branchTests ? `Use ${cfg.branchTests} for relevant changes.`
    : "Run the project's own tests for what you changed, and report which.";
  return `Hard rules: never work on the main branch; everything you commit is English. ${tests}
Keep your own context small: start from Touches and the task's own documents. Search by path or
symbol with bounded output; read relevant ranges. Keep only conclusions from delegated research.
Implement only Acceptance within Scope. Once verified, report done and stop. After two failed
approaches, report the evidence and blocker; stop instead of broadening the task. Aim for one
focused work session of about 30 minutes; if more remains, report DONE, NEXT, FILES, TRAPS.
Re-run verification only after a relevant change or a new failure; the full suite belongs to the gate.`;
}

const PROTOCOL = `Protocol: STATE YOUR REPORT OR QUESTION AS YOUR FINAL MESSAGE AND STOP.
The conductor reads it and resumes this session when needed. Do not message the conductor directly.
Report outcome (done, blocked, or partial), changed files, verification, and the next step in at most 15 lines.`;
...
// an ordinary task.
export const modelFor = workerRole;
...
${hardRules(cfg, runtime)}`);
  ...
  if (model === 'review') parts.push(hardRules(cfg));
  const rules = projectRules(cfg);
  if (rules) parts.push(rules);
  if (cfg.briefExtra) parts.push(cfg.briefExtra.trim());

  parts.push(`Write questions and reports in ${cfg.language}; committed content is English.`);
