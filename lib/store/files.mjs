...
export function makeFileStore(cfg) {
  ...
  // One helper for every call here, read or write. There used to be a second one, `gitWrite`,
  // that stamped `ORCHESTRA_GATE: '1'` on the two calls that write (`add`, `commit`), on the belief
  // that the marker was what let `hooks/guard-main-commit.mjs` allow the plugin's own writes to
  // main. It never did: `add`/`commit` here run through `execFileSync('git', …)` directly, never
  // through the Bash tool, so no PreToolUse hook has ever fired on them in the first place — the
  // same conclusion `hooks/guard-main-commit.mjs`'s own header and `lib/gate/land.mjs`'s `gateEnv`
  // both state about their own git calls. The marker had no reader, and removing it leaves nothing
  // left to distinguish the two helpers.
  ...
  return {
    ...
    publish(draftPath) {
      ...
      if (staged.trim()) git('commit', '-q', '-m', `roadmap: publish ${slug}`, '--', target);
