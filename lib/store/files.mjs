...
export function makeFileStore(cfg) {
  ...
  // gets none — only the two calls that actually write (`add`, `commit`) carry ORCHESTRA_GATE,
  // marking a write the plugin itself makes on the main branch so `guard-main-commit`
  // (hooks/guard-main-commit.mjs) can let it through without exempting git generally. Putting the
  ...
  const gitWrite = runGit({ ...process.env, ORCHESTRA_GATE: '1' });
