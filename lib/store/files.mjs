...
export function makeFileStore(cfg) {
  ...
  // frontmatter names none — the same rule `publish()` uses for the slug it writes under. `drafts()`
  // applies it too, so a draft named `foo.md` that declares `roadmap: bar` can never be listed as
  // `foo` and published as `bar.md`: that would make the filename and the frontmatter two sources
  // for the same fact, free to disagree.
  ...
  return {
    ...
    drafts: () => mdIn(draftsDir).map((f) => ({ slug: draftSlug(f), path: join(draftsDir, f) })),
