// The only place in the plugin that talks to GitHub, and the only GitHub caller that imports
...
export function makeGh({ run = defaultRun } = {}) {
  ...
  const flat = (i) => ({
    ...
    // A roadmap's owner is the author of its programme issue (./ownership.mjs). null
  ...
  return {
    ...
    // assign's counterpart: `--add-assignee` only ever ADDS, so a release that never called this
    // left the previous holder's name in place forever — the next claimant's own `--add-assignee`
    // could not displace it, and the success check reading the assignee back found the wrong name.
    unassign: (n, who) => run(['issue', 'edit', String(n), '--remove-assignee', who]),
    ...
    // Its counterpart, and the reason it exists: in planetCraft, `planProgrammes`
    // (`tools/roadmap/sync.mjs`) closes a programme when its last task closes, and skips a closed
    // programme for ever after. So publishing a NEW task into a roadmap that had finished used to
    // leave the programme shut — a finished roadmap holding unfinished work, which is what the
    // monitor and every reader would then see. The closing half is a later phase here; this half
    // ships now because `publishIssues` is what meets a closed programme. It is the only caller;
    // nothing else here reopens anything.
