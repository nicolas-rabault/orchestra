...
//
// Gated on the STRICT status (`=== 'landed'`), never on `isLanded` (which also admits `UNVERIFIED`,
// board.mjs's `'landed?'`). A status label is a projection of the derivation, and the derivation
// for an unverified row is deliberately not `landed` — that is the whole reason `UNVERIFIED` is a
// separate word rather than a synonym. Stripping `status:todo` off such a row would leave the shared
// channel with an open issue carrying no `status:` label at all — invisible to anyone filtering
// `status:todo`/`status:wip`, nothing having closed it, and only a LOCAL `unverified:` line to
// explain it (board.mjs: 22 of 41 landed rows in planetCraft carried `subjects: []`, measured
// 2026-08-20, so this is the common case, not an edge one). Leaving the label on an unverified row is
// a shared channel that is
// merely behind the truth; stripping it on the strength of `isLanded` would make it wrong.
