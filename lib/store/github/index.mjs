...
import { LABELS, keyFromTitle, publishIssues, programmeReport } from './issues.mjs';
...
export function makeGithubStore(cfg, { gh: injected, run } = {}) {
  ...
  return {
    ...
    publish(draftPath) {
      ...
      // `report` says what happened to the PROGRAMME, which is the half of a publish nothing
      // else reconciles: a created programme and a completed one used to print identically.
      return { slug, keys: parsed.tasks.map((t) => t.key), ref: res.programme, report: programmeReport(res) };
    },
