...
export function makeGithubStore(cfg, { gh: injected, run } = {}) {
  ...
  // `bar`. Mirrors `files.mjs`'s own `draftSlug`; not shared between the two stores because doing
  // so would mean one importing the other's internals for four lines.
  ...
  return {
    ...
    drafts: () => (existsSync(draftsDir) ? readdirSync(draftsDir) : [])
      ...
      .map((f) => ({ slug: draftSlug(f), path: join(draftsDir, f) })),
