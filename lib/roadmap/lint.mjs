...
export function lintRoadmap(parsed, {
  ...
  for (const t of parsed.tasks) {
    ...
    for (const d of t.deps) {
      ...
      if (!(qualified ? knownKeys.has(d) || parsed.tasks.some(other => other.key === d) : localIds.has(d)))
    ...
    if (!t.why?.trim()) at(t.line, `${t.id}: no paragraph beginning "**Why"`);
    if (!t.acceptance?.trim()) at(t.line, `${t.id}: no paragraph beginning "**Acceptance"`);

    if (t.scope !== undefined) {
      if (!t.scope.trim()) at(t.line, `${t.id}: Scope must state the boundary of this task`);
      if (t.touches.length > 5) at(t.line, `${t.id}: scoped tasks allow at most 5 Touches paths; split the result`);
      if ([t.why, t.acceptance, t.scope].join(' ').trim().split(/\s+/u).length > 180)
        at(t.line, `${t.id}: scoped task exceeds 180 words; simplify the result`);
    }
    if (t.key) {
  ...
  // Resolve qualified references to this same roadmap before checking local cycles.
  const byKey = new Map(parsed.tasks.filter(t => t.key).map(t => [t.key, t]));
  const done = new Set(), active = new Set();
  function visit(t) {
    if (active.has(t.key)) { at(t.line, `${t.id}: dependency cycle; no task in this cycle can start`); return; }
    if (done.has(t.key)) return;
    active.add(t.key);
    for (const dep of t.deps) {
      const target = byKey.get(dep.includes('/') ? dep : `${t.roadmap}/${dep}`);
      if (target) visit(target);
    }
    active.delete(t.key);
    done.add(t.key);
  }
  parsed.tasks.forEach(visit);
  return out;
}
