...
//
// THE HOLE THIS CLOSES, and it had been open since the register existed. `.orchestra/
// state.json` has exactly three writers: planetCraft's `tools/roadmap/register.mjs` (edits ONE
// existing row at a landing), `lib/register/archive.mjs` (drops prose from terminal rows) — and
// the conductor, an LLM, rewriting the whole file by hand. NOTHING has ever CREATED a row. The
// only instruction that says to create any is the orchestra skill's "Adoption" section, and its
// own first line is "first run, or state lost": it builds the table from the board ONCE and then
// records `adopted: true`. So a roadmap published after that moment reaches GitHub, reaches the
// board, and reaches nothing else. `lib/register/ready.mjs` schedules off the register, so the
// whole programme looks idle while every one of its tasks is ready to start.
//
...
export function enrol(state, tasks, { at, host, foreign = () => false }) {
  ...
  for (const task of tasks) {
    ...
    // it rather than write `null` into the register's identity column, which `lib/register/ready.mjs`
    // would then throw on for the whole tick.
    ...
    const note = alien
      ...
      : `[ENROLLED ${at} ${host}] Enroled by \`roadmap publish\`. Status not reconciled yet: the next tick's reconcileTasks() reads git.`;
