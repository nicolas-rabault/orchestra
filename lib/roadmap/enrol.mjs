...
// `tools/orchestra/ready.mjs` schedules off the register — this plugin's own scheduler,
// `lib/register/ready.mjs`, reads the same column — so the whole programme looks idle while every
// one of its tasks is ready to start.
//
...
export function enrol(state, tasks, { at, host, foreign = () => false, mine = () => true }) {
  ...
  for (const task of tasks) {
    ...
    // `tools/orchestra/ready.mjs` throws on it for the whole tick, and this plugin's own
    // scheduler, `lib/register/ready.mjs`, reads the same column.
    ...
    appended.push(registerRow(task, { roadmapSlug: task.roadmap, status, note, mine: mine(task) }));
    added.push(task.key);
  }
