...
// unreadable: ten lines, of which one was real. `derived` here is the LOCAL derivation — see the
// call site for why it may not be the overlay-aware one.
...
// stayed open. `orchestra roadmap sync` (`lib/store/github/index.mjs`'s `sync`) closes exactly this
// case when it runs — but nothing runs it automatically, so between one sync and the next, this
// line is what tells a human the issue is still open. Named rather than folded into a generic
// disagreement, because the action is specific and it is OUTWARD-FACING: closing someone's issue is
// not a local edit, and no tool here does it on its own initiative.
...
export function reconcile({ tasks, git, register = [], overlay = new Map() }) {
  ...
  const statused = tasks.map((t) => {
    ...
    return {
      ...
      // Whether a shared channel carries this row at all — which is the only honest way to ask
      // "could a claim on it ever have been recorded". Offline, and for a `destination: local`
      // roadmap in ANY mode, `claim` returns `{ ok: true }` having written nothing
      // (lib/store/files.mjs) because there is nobody to tell, so no claim can reach the board and
      // `startVerdict` (./policy.mjs) was demanding evidence the row's own store cannot produce: on
      // 2026-09-07 the worktree guard refused `pr/PR91` one second after `orchestra roadmap claim
      // pr/PR91` reported success, and told the conductor to re-run the command it had just run.
      // Read off the overlay's PRESENCE, not off `issue` being null: the entry is the fact, an
      // issue number is one field of it, and a store that grew an entry without a number would
      ...
      landedHere: deriveLocalStatus(t, git, reg?.subjects ?? []) === 'landed',
    };
  });
  ...
  // A private copy of the register's TERMINAL vocabulary (lib/register/archive.mjs), and
  // deliberately not imported from there: this module derives status from git and treats the
  // register only as an orphan-detection input, never as a source of truth, so reaching across
  // that seam would tie a roadmap-derivation module to the register's internals for one Set.
