...
// stayed open. `lib/roadmap/sync.mjs` closes such an issue automatically — but only off the
// recorded subjects, so the recording gap above disables it too. Named rather than folded into a
// generic disagreement, because the action is specific and it is OUTWARD-FACING: closing someone's
// issue is not a local edit, and no tool here does it on its own initiative.
...
export function gatherGit(root, { mainBranch = 'main', run } = {}) {
  ...
  // A `mainBranch` with no commits yet — a fresh repository, or a default branch nobody has
  // committed to — is not a broken repository; it is a project with no landed history, and the
  // honest answer is an empty set. Anything else git might fail on (the binary missing, a
  // corrupted `.git`, a permission error) does not carry this message, and is left to throw: a
  // broken repository must still be loud.
  let mainSubjects;
  try {
    mainSubjects = git('log', '--format=%s', mainBranch);
  } catch (e) {
    if (/unknown revision|does not have any commits yet/.test(e.stderr ?? e.message ?? '')) mainSubjects = '';
    else throw e;
  }
  return { refs, mainSubjects: new Set(mainSubjects.split('\n').map((s) => s.trim()).filter(Boolean)) };
}
