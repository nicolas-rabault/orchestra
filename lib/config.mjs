...
export const DEFAULTS = {
  ...
  // `local` is a THIRD destination, not a third mode: a roadmap declares it in its own frontmatter
  // (lib/roadmap/parse.mjs), so a project keeps publishing its development roadmaps as issues or as
  // committed markdown while its `pr` roadmap publishes to neither. Gitignored, because a review
  // run must add nothing to the repository it is reviewing.
  roadmaps: { drafts: '.orchestra/drafts', published: 'docs/roadmaps', local: '.orchestra/roadmaps' },
