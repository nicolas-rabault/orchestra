...
// `lib/register/state.mjs`'s `writeState` and `lib/register/lock.mjs`'s `acquire` already use, and
// for the measured reason they name: a run record has one writer at a time but several readers that
// may poll mid-write, and `writeFileSync` truncates before it writes, so a reader can land on an
// empty or half-written file. `parseRun` correctly maps that unreadable file to `null` — the same
// "unreadable means no record" discipline `parseState` uses — which `runVerdict` reads as `unknown`.
//
// Caught in this file's own acceptance run: one in roughly thirty runs of
// `test/p3-acceptance.test.mjs` had `awaitRun`'s FIRST read (fired immediately after `detach`
// returns, before any poll sleep) land inside `recordOutcome`'s pid-claim write in the just-spawned
// child — the two are correlated by startup timing, not spread evenly across the 2-second polls,
// which is why the failure looked rare rather than absent. `rename` is atomic on every filesystem
// this runs on, so a reader now sees either the old record whole or the new one whole, never half.
...
export function queueList(cfg) {
  ...
  // `queue.json` already has one guarded writer — `land.mjs`'s `mark`/`withMutex` — and this is an
  // unguarded second one: a landing marking an entry while `queue-list` reaps a vanished branch is
  // the same torn-read window as the run record above, just on a different file.
  writeAtomic(p.state, S.formatState(state));
