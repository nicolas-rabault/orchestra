...
// (spec §5) — the one marker for "the plugin itself is writing to main on purpose", the same one
// `lib/store/files.mjs`'s `gitWrite` stamps on its two git writes — and ORCHESTRA_FULL_SUITE is
// spec §10's override for `guard-full-suite`, the deliberate whole-suite run that hook exists to
// make deliberate.
//
...
// environment; and from `process.env`, for the case where a configured gate is itself an agent
// session whose hooks inherit this environment. Neither reader covers THIS file's own
// `execFileSync('git', …)` calls below — those never go through the Bash tool, so no hook ever
// fires on them, and the marker was never what let the gate write to main.
  ...
  for (const gate of gates) {
    ...
    if (r.status !== 0) {
      ...
      const why = r.error?.code === 'ETIMEDOUT'
        ...
        : r.signal
          ? `gate "${gate.name}" refused (killed by ${r.signal})`
