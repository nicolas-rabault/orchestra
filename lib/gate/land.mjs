...
// The environment every git call inside a landing runs under — reads and writes alike, so a reader
// never has to work out which of the gate's git calls are "the gate's own".
//
...
function commitLedgers(cfg) {
  ...
  const ok = gitOk(cfg.root, ['add', '--', ...ledgers])
    && gitOk(cfg.root, ['commit', '--no-verify',
