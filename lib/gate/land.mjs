...
import { appendLine, readJsonl } from '../jsonl.mjs';
...
// The names alone, creating nothing. A READER must not have to write to ask a question: `ready` runs
// this on every tick of every project, including ones that have never landed a branch, and
// `gatePaths` below would leave two empty directories behind each time just for being asked where
// the queue lives.
...
function landing(cfg, branch, waitSeconds) {
  ...
  const freeBytes = (() => {
    try { const s = statfsSync(wt.path); return s.bavail * s.bsize; } catch { return null; }
  })();
