...
//
// THE WORKTREE ALWAYS GOES, AND THE NOTE SAYS WHAT WENT WITH IT. It used to be kept whenever the
// tree was dirty: tracked-dirty was pre-empted, and an untracked non-ignored file made `git worktree
// remove` refuse on its own (exit 128 with nothing removed — measured on git 2.52.0, 2026-08-31, in
// scratch repositories, where an IGNORED directory never did). Both readings were wrong HERE, and
// the reason is where this function runs. The landing's own precondition proved this very worktree
// clean, under the lock, minutes earlier; the only writers since are the rebase and the gates. So
// what was being preserved was a coverage report or a formatter's rewrite, and the price was the
// worktree AND — still checked out in it — the branch that had just landed: two dead things per
// landing, removable only by a hand that first had to read a note. Hence --force, every time, and a
// note that NAMES every file destroyed, which is the trace the old refusal was really providing.
// `dirtyFiles` is therefore asked eagerly, unlike the thunks below: it is not a fallback, it is the
// last moment at which what is about to be destroyed can still be named.
//
...
export function cleanupAfterLanding({
  branch, worktree, dirtyFiles, removeWorktree, deleteBranch, provenMerged, unsetUpstream,
}) {
  const notes = [];
  const dirty = dirtyFiles();
  const removed = removeWorktree();
  if (removed && dirty.length) {
    notes.push(`worktree ${worktree} removed with ${dirty.length} uncommitted file(s)`
      + ` the gates left behind: ${dirty.join(', ')}`);
  }
  if (!removed) notes.push(`worktree ${worktree} kept — git refused to remove it, remove it by hand`);
  ...
  if (!removed) { notes.push(`branch '${branch}' kept — still checked out in ${worktree}`); return notes; }
...
// Whether a landing must move the LOCAL main branch onto the commit the remote holds, before it
// rebases the branch onto it. Three inputs and no git: the tracking ref's name (null when the main
// branch tracks nothing), how many commits that upstream holds which the main branch does not, and
// whether the main checkout has an uncommitted change to a tracked file.
//
// `skip` is the answer that matters most, because it is the ordinary one. A project whose main
// branch tracks nothing never reaches a fetch at all, and a main branch that is not behind is left
// untouched — so a landing in a repository whose remote has not moved runs exactly the sequence it
// ran before this decision existed, dirty checkout included.
//
// `refuse` is a narrowing, and a deliberate one. A landing has always tolerated an uncommitted
// change in the main checkout as long as the branch did not touch the same paths (`clashingPaths`);
// a rebase cannot, because git needs the tree clean to replay commits into it. Rather than stash on
// somebody's behalf, or land on a base the remote has already left behind, the landing stops and
// names the fix — and only ever in the moment the upstream really moved.
export function syncMainDecision({ upstream, behind, dirty }) {
  if (!upstream || behind === 0) return 'skip';
  return dirty ? 'refuse' : 'rebase';
}
...
export const EXIT = {
  ...
  vanished: 16,
  // THE MACHINE REFUSED, NOT THE BRANCH. Distinct from 11 for the reason 11's own hole is not: 11
  // folds "dead code" and "a test failed" because both are the branch author's to fix and the agent
  // does the same thing with either. This is a different actor. The gate's process was killed before
  // it reached any verdict on the branch, or the machine could not run it at all — and the agent's
  // move is not "report to the author", it is "fix the machine, or land again once".
  //
  // Measured 2026-09-08 in duckJam, three shapes in one day, each costing a full gate run and a
  // wrong diagnosis before anybody understood it: four innocent branches refused on one wall-clock
  // assertion at load 170 (ticket `t-0a1q330`) with the gate itself killed at 54% "without a failed
  // test or a summary — a refusal indistinguishable from a real failure" (`t-17amowz`); 228 errors
  // charged to CD7, innocent, all `OSError [Errno 28] No space left on device` (`t-17doeah`); and a
  // SIGABRT inside onnxruntime's telemetry at teardown, twice, pytest dying with neither summary nor
  // traceback (`t-0cvdkrg`). What all three share is that the barrier produced NO VERDICT. The plugin
  // made them indistinguishable from a barrier that named failing tests; this number is the
  // distinction.
  machine: 17,
};

// ---------------------------------------------------------------------------------
// WHO REFUSED: the branch, or the machine.
// ---------------------------------------------------------------------------------

// A shell hides a signal. Every gate runs through `spawnSync(cmd, { shell: true })` — a project's
// gate is a command line, not an argv — so when the gate's own process is killed, the SHELL exits
// normally with 128+N and `spawnSync` reports `signal: null`. `land.mjs` used to read only
// `r.signal`, which is therefore null for every real case: a SIGABRT out of a native library and an
// OOM kill both arrived as a plain non-zero status and were charged to the branch.
//
// Only the signals a killed gate actually dies of are listed. 128+N is a convention, not a
// guarantee — a program may exit 137 because it chose to — so an unlisted number stays the branch's
// refusal rather than being read as a signal on the strength of arithmetic.
const KILLED_BY = { 2: 'SIGINT', 6: 'SIGABRT', 7: 'SIGBUS', 9: 'SIGKILL', 11: 'SIGSEGV', 15: 'SIGTERM' };

// What one gate's non-zero result means. Pure: `land.mjs` hands over what `spawnSync` returned.
//
// A TIMEOUT STAYS THE BRANCH'S, deliberately. It is the one ambiguous case — a loaded machine and a
// branch that hangs produce the identical kill — and the two errors do not cost the same: reading a
// hang as the machine's sends the agent to land it again, for ever, on a gate that will never
// finish. A configured timeout is also a number the project chose, which is evidence a signal is
// not.
export function gateOutcome({ name, timeout = null, status = null, signal = null, errorCode = null }) {
  if (errorCode === 'ETIMEDOUT')
    return { actor: 'branch', exit: EXIT.refused, note: `gate "${name}" refused (killed after ${timeout}s)` };
  if (signal)
    return { actor: 'machine', exit: EXIT.machine, note: `gate "${name}" was killed by ${signal} — no verdict on the branch` };
  const killed = Number.isInteger(status) && KILLED_BY[status - 128];
  if (killed)
    return { actor: 'machine', exit: EXIT.machine, note: `gate "${name}" exited ${status}: its process was killed by ${killed} — no verdict on the branch` };
  return { actor: 'branch', exit: EXIT.refused, note: `gate "${name}" refused` };
}

// How little free space is too little to start a landing on. One gibibyte.
//
// Not a guess about what a project's gates need — the plugin cannot know that — but the floor below
// which any test suite is in trouble on any project. Measured 2026-09-08 in duckJam: a full disk
// produced 228 `OSError [Errno 28] No space left on device` inside a pytest run, all 228 charged to
// branch CD7, which was innocent; the conductor diagnosed the branch twice before it read one of the
// error lines. A landing refused here costs the caller a `df` and a retry; a landing let through
// costs a full gate run, a wrong verdict, and a person's afternoon.
export const MIN_FREE_BYTES = 1024 * 1024 * 1024;

const gib = (n) => `${(n / (1024 ** 3)).toFixed(2)} GiB`;

// The refusal, or null to go on. `free` is null when the filesystem could not be asked at all, which
// runs the landing: a check that cannot answer must not become a check that refuses.
export const diskRefusal = (free, where) => (free === null || free >= MIN_FREE_BYTES ? null
  : `${where} has ${gib(free)} free, under the ${gib(MIN_FREE_BYTES)} a gate run needs`
    + ' — free some space and land again. The branch was not touched and is not at fault.');

// ---------------------------------------------------------------------------------
// EVERY PASSAGE, not the last one.
// ---------------------------------------------------------------------------------
//
// `runs/<branch>.json` is one file per BRANCH, rewritten by every attempt, and that is correct for
// what it is: `await` needs the record of the landing running right now, and a stale predecessor
// would be worse than none. What it cannot do is answer "how many times has this branch been
// refused, and why" — the question a retrospective asks first. Measured 2026-09-09 across duckJam's
// 108 run records: 107 read `exit: 0` while the journal documents at least eight refusals, one
// branch landing "at the fourth attempt". The ledger had overwritten its own evidence.
//
// So every finished attempt appends one line here, and nothing ever rewrites one. It is also what
// makes EXIT.machine safe to act on: "land again once" is only a rule an agent can follow if
// something can tell it whether it already did.
export const attemptLine = ({ branch, startedAt, endedAt, exit, note = '' }) =>
  JSON.stringify({ branch, startedAt: new Date(startedAt).toISOString(), endedAt: new Date(endedAt).toISOString(), exit, note });

// What the ledger says about one branch: how many attempts, and the outcome of each, oldest first.
// `entries` is whatever `lib/jsonl.mjs`'s reader returned — a malformed line is not this function's
// problem and never reaches it.
export function attemptSummary(entries, branch) {
  const mine = entries.filter((e) => e?.branch === branch);
  if (!mine.length) return null;
  const secs = (e) => Math.max(0, Math.round((Date.parse(e.endedAt) - Date.parse(e.startedAt)) / 1000));
  return {
    count: mine.length,
    refusals: mine.filter((e) => e.exit !== EXIT.ok).length,
    // Oldest first, so the line reads as a history: `11, 17, 0`.
    outcomes: mine.map((e) => ({ exit: e.exit, note: e.note ?? '', seconds: secs(e) })),
  };
}

// One line, for `queue-list` and for `await`'s report. Null when this branch has no history worth
// printing — a first attempt is not a history.
export function attemptHistoryLine(summary) {
  if (!summary || summary.count < 2) return null;
  const past = summary.outcomes.slice(0, -1);
  return `attempt ${summary.count}; before this one: `
    + past.map((o) => `exit ${o.exit}${o.note ? ` (${o.note})` : ''} after ${o.seconds}s`).join(', ');
}

// ---------------------------------------------------------------------------------
// A BRANCH NOBODY IS MOVING.
// ---------------------------------------------------------------------------------
//
// `held` and `blocked` with no live process are the durable record doing its job: a branch a gate
// refused, or one a conflict stopped, is waiting for its author and will wait as long as it takes.
// `queued` and `landing` are the opposite — both mean a process was going to move this branch — so
// either of them with no live process behind it is a landing that died and took its own memory with
// it.
//
// Measured 2026-09-08 in duckJam: the gate merged LP2 and its process died before releasing, and LP6
// sat `queued` for two hours with nothing anywhere naming it. `queue-list` would have said so — its
// "(no live process)" column has always been correct, and this is that same reading, lifted out so
// `orchestra ready` can print it on a tick without anybody having to think to go and look.
//
// NOT a reaping rule, deliberately, and the retex that found LP6 proposed one: `queue.json` carries
// no pid, so an entry whose process died is reaped by nobody. Verified against the code before
// acting on it — and it changes nothing, because `queue.json` takes no part in granting the lock.
// `acquire` decides from `holder` and `waiters` alone (`mayTake`), and both are pid-reaped on every
// pass. A phantom entry of any state cannot make a second branch wait; what it can do is sit there
// unnamed, which is the actual harm and what this fixes.
//
// `holder` and `waiters` must already be reaped; this function has no opinion about liveness.
export const stalledEntries = ({ entries, holder, waiters }) => entries.filter((e) =>
  (e.state === 'queued' || e.state === 'landing')
  && holder?.branch !== e.branch
  && !waiters.some((w) => w.branch === e.branch));
