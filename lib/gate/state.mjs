...
export function runVerdict({ run, alive, nowSec }) {
  ...
  // one that has gone missing. Measured without this case: a seed read in that window came back
  // `vanished` (exit 16) to BOTH its readers — `await` reported a killed landing and told the caller
  // to check `git log` "before starting over", the worst instruction at that instant, and `detach`'s
  // own guard (which reads `running` to refuse a second start) let a second child through, orphaning
  // the first.
  if (run.pid === null) return { state: 'running', exit: EXIT.busy, elapsed: nowSec - run.startedAt };
