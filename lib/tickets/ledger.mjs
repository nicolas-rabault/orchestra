...
//
// `fingerprint` is the source project's `crash` kind, renamed: the source validated it against a
// format tied to ONE reporter (`errorFingerprint.js`'s specific 7-base36-character output), and
// this plugin has to accept the token any project's own crash/error reporter computes, not just
// that one shape. So the stored key prefix is `fingerprint:`, not `crash:` — a ledger this plugin
// writes and one the source project wrote use different prefixes for the same kind of
// observation, though both still load fine (`loadTickets` never re-derives a fingerprint from a
// stored row, it only reads one back).
export function fingerprintFor(obs = {}) {
  ...
  if (named[0] === 'fingerprint') {
    const fp = String(obs.fingerprint).trim().toLowerCase();
    // A short alphanumeric token — at most 12 characters, the shape a project's own reporter
    // computes (planetCraft's errorFingerprint.js returns 7 base36 characters; a hex hash, a
    // short base36/base62 digest and similar all fit the same cap). REJECTED rather than folded
    // like prose: a caller passing the raw message or stack instead of a real token would fold
    // NOTHING, since both carry exactly the line numbers and details a fingerprint exists to
    // discard, and every occurrence would then mint a fresh ticket — the "queue nobody reads"
    // failure this whole module exists to prevent, arriving through the one input this function
    // does not get to slugify away.
    if (!/^[0-9a-z]{1,12}$/.test(fp)) {
      throw new Error(`tickets: "${obs.fingerprint}" is not a canonical fingerprint`
        + ' — pass the short token your own crash/error reporter computed, not the message or the stack');
    }
