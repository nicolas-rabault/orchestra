...
//
// Deliberately no leg here for the page itself. `port` and `monitorPid` left this registry
// entirely — the one page's claim lives in `~/.orchestra/monitor.json`, and is checked on its own
// by `lib/cli/monitor.mjs`'s `pageAlreadyUp` (a live pid AND an actual listener, never one alone).
// A page left open for days must not pin a dead conductor's whole entry — worker count included —
// alive for as long as its browser tab stays open, so nothing about the page may count here as
// proof that the PROJECT is alive.
...
// full replace was fine while `ready` was this file's only writer; the machine-wide monitor makes
// `discoverProjects` (`lib/monitor/discover.mjs`) a second one, writing `id`, `name`, `root` and
// `mode` onto the SAME entry the first time it finds the current directory's project unregistered
// — while `ready` writes the worker side (`workers`, `conductorPid`, …) — and a replace would let
// each erase the other's fields. The dangerous direction is `discoverProjects` erasing `workers`:
// another project would then read `undefined`, count 0, and launch MORE — the wrong direction for
// an advisory cap. Each writer sends only the fields it owns; both send `id`, `name`, `root`,
// `mode` identically.
//
...
// never `updatedAt`, which a write that carries no `workers` at all can still move.
// `discoverProjects` is exactly that: the first request (or `orchestra monitor` invocation) that
// finds the current directory's project unregistered calls `recordInstance` with only `id`,
// `name`, `root` and `mode`, restamping `updatedAt` with no worker count behind it. Coupling the
// count to `updatedAt` would let that write alone revive a `-9`-killed conductor's stale count for
// another `SEEN_STALE_MS`. An entry written before this change carries no `workersAt` at all; it
// falls back to `updatedAt`, which is exactly today's behaviour.
