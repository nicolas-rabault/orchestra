...
// looked for `-f`. Here the drafts directory is gitignored only once `orchestra init` has written
// that line, so a project that has not run `init` yet, or has since edited its own `.gitignore`,
// has no such floor: an ORDINARY `git add` must be caught too, `-f` or not.
//
// What this cannot catch: `git add -A` (or a bare `git add .`) sweeps a draft into the index
// without ever NAMING it, and this hook has no path to inspect there — only the gitignore stops
// that case.
import { segments, stripEnv } from './segments.mjs';
