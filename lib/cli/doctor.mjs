...
// The two things about a project's gates that are true, cost minutes a day, and that nothing
// anywhere said out loud. Both were measured on 2026-09-09 in duckJam.
//
// 1. THE SUITE GUARD KEYS ON A GATE NAMED `suite`. `guard-bash.mjs`'s full-suite rule refuses a
//    bare run of `gates.find(g => g.name === 'suite').cmd` and nothing else, so a project whose
//    gate is called anything else has never once been guarded — duckJam calls its only gate
//    `pytest`, and every worker there was free to run the whole suite out of habit on a machine
//    forty of them share. Silence is the wrong way to deliver that: the guard reads, in the
//    config, exactly like one that works.
//
// 2. NO GATE DECLARES `skipWhenAllPathsMatch`, so every landing runs everything. It is already
//    implemented and tested (`gateSkipped`, lib/gate/state.mjs) and documented in the README, and
//    not one project uses it — while duckJam's six successful landings cost 135 minutes of gate
//    time, a full pytest run each, several of them for branches that touched only documents or
//    rendered images.
//
// Neither line changes any behaviour. They are `doctor`'s job: naming what a human is looking for
// when they wonder where the day went.
function gatesAdviceLine(cfg) {
  if (!cfg.gates.length) return null;
  const out = [];
  if (!cfg.gates.some((g) => g.name === 'suite')) {
    const names = cfg.gates.map((g) => g.name).join(', ');
    out.push(`  ${pad('suite guard', 20)} INACTIVE — it refuses a bare run of the gate named \`suite\`, and this project's`
      + `\n  ${' '.repeat(20)} gates are named ${names}. Rename the full-suite one to \`suite\` to have agents`
      + `\n  ${' '.repeat(20)} kept off it between landings.`);
  }
  if (!cfg.gates.some((g) => Array.isArray(g.skipWhenAllPathsMatch) && g.skipWhenAllPathsMatch.length)) {
    out.push(`  ${pad('gate skips', 20)} none — every landing runs every gate, whatever the branch changed.`
      + `\n  ${' '.repeat(20)} Give a gate \`skipWhenAllPathsMatch\` (e.g. ["docs/**", "**/*.md"]) and it is`
      + `\n  ${' '.repeat(20)} skipped when EVERY changed path matches one of them.`);
  }
...
export function doctorText(cfg) {
  ...
  const rows = [
    ['conductor runtime', conductorRuntime],
    ['roadmapRuntimes', Object.entries(cfg.roadmapRuntimes ?? {}).map(([slug, runtime]) => `${slug}=${runtime}`).join(', ') || 'inherit conductor runtime'],
    ['conductor heartbeat', conductorRuntime === 'codex' ? 'native Codex automation; legacy Claude tick disabled' : 'Claude heartbeat'],
    ...
    ['maxAutoResumes', cfg.maxAutoResumes],
    ...Object.entries(cfg.workerModels ?? {}).map(([k, v]) => [`workerModels.${k}`, v ?? 'native default']),
    ...Object.entries(cfg.workerThinking ?? {}).map(([k, v]) => [`workerThinking.${k}`, v ?? 'native default']),
    ['retireAt', `${Math.round(cfg.retireAt / 1000)}k tokens of prefix`],
  ...
  const stale = cfg.monitor?.port !== undefined
    ...
    : '';
  const extra = [heartbeatLine(cfg), gitignoreLine(cfg), rerereLine(cfg), traceLine(cfg), gatesAdviceLine(cfg), heartbeatAgentsLine()]
    .filter(Boolean).join('\n');
