...
export function readyCommand({ cfg, args }) {
  ...
  const planned = planLaunches(ready, inFlight, effective);
  const liveness = gatherLiveness(cfg.root, tasks);
  const { idle, undelivered, driving, relaunch } = obligations(tasks, liveness);
  ...
  const held = launchHold(planned, { idle, undelivered });
  ...
  if (args.includes('--json')) {
    out(JSON.stringify({ corrections, tasks, blocked, ready, launches, launchHeld: held,
      undelivered, idle, driving, relaunch, waiting, stalled, retire, fleet,
