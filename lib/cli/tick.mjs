...
export function readyCommand({ cfg, args }) {
  ...
  // `lib/cli/monitor.mjs` writes `{port, monitorPid}` onto that same entry — `recordInstance`
  // MERGES the two field sets (`lib/machine.mjs`) rather than one replacing the other, so the two
  // writers never clobber each other's half.
  //
  ...
  // `updatedAt` for an entry written before this change — because the monitor's own keepalive
  // (§8.3, `lib/cli/monitor.mjs`) moves `updatedAt` with no fresh worker count behind it, and
  ...
  // launches, never too many". `beatAt` and `conductorSession` are the content spec §8.2 declares
  // for the registry's entry, filled here and printed by `orchestra instances`
  // (`lib/cli/monitor.mjs`) — a truncated session id and a relative beat age beside the worker
  // count they explain.
