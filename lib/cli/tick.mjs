...
export function readyCommand({ cfg, args }) {
  ...
  // one writer of the WORKER side of the registry entry. It will not stay the file's only writer:
  // P4's monitor (not yet built) will write `{port, monitorPid}` onto that same entry once it
  // exists — `recordInstance` already MERGES the two field sets (`lib/machine.mjs`) rather than one
  // replacing the other, ready for that second writer before it arrives.
  //
  ...
  // `updatedAt` for an entry written before this change — because once the monitor exists, its own
  // keepalive (§8.3) will be able to move `updatedAt` with no fresh worker count behind it, and
  // coupling the count to that stamp would freeze a `-9`-killed conductor's count alive for as long
  // as its browser tab stayed open. An undercounted project would make every OTHER project launch
  // MORE, which is the wrong direction for an advisory cap — this file's header calls it "too few
  // launches, never too many". `beatAt` and `conductorSession` are still read by nothing so far in
  // this phase: they are the content spec §8.2 declares for the registry's entry, filled here for
  // this phase's own `orchestra instances` to display once it is written — not dead fields, just
  // not yet read.
