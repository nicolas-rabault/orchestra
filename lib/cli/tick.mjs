...
import { cleanupWorktrees } from '../register/cleanupWorktrees.mjs';
...
export function readyCommand({ cfg, args }) {
  ...
  const cleanup = cleanupWorktrees(cfg, tasks);
  ...
  const planned = planLaunches(ready, inFlight, effective, { ...cfg, conductorRuntime: state.conductor?.runtime ?? 'claude' });
  const liveness = gatherLiveness(cfg.root, tasks);
  const { idle, undelivered, driving, relaunch, refresh = [], limited = [] } = obligations(tasks, liveness, { maxAutoResumes: cfg.maxAutoResumes });
  ...
  if (args.includes('--compact')) {
    const brief = t => ({ id: t.id, status: t.status, session: t.session ?? null });
    out(JSON.stringify({ corrections, cleanup, launches, blocked, launchHeld: held,
      idle: idle.map(e => ({ ...brief(e), lastWords: e.turn ? lastWords(e.turn.log) : null })),
      undelivered, relaunch, refresh, limited, waiting, stalled, retire,
      driving: driving.map(brief), errors: liveness.errors,
      budget: { effective, inFlight } }));
    return;
  }
  if (args.includes('--json')) {
    out(JSON.stringify({ corrections, cleanup, tasks, blocked, ready, launches, launchHeld: held,
      undelivered, idle, driving, relaunch, refresh, limited, waiting, stalled, retire, fleet,
  ...
  for (const c of cleanup.removed) out(`CLEANED: ${c.id} — ${c.path}`);
  for (const c of cleanup.kept) out(`CLEANUP HELD: ${c.id} — ${c.path}: ${c.reason}`);
  ...
  for (const e of limited) out(`LIMIT: ${e.id} — resume budget reached; inspect the report, split or narrow before renewing`);
