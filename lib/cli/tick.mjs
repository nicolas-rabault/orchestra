...
export function readyCommand({ cfg, args }) {
  ...
  // one writer of the WORKER side of the registry entry; it is no longer the file's only writer
  // since P4, whose monitor writes `{port, monitorPid}` onto that same entry — `recordInstance`
  // MERGES the two field sets (`lib/machine.mjs`) rather than one replacing the other.
  //
  ...
  // clock on every write (the monitor's included), so an entry never reads dead the instant it is
  // written just because nobody happens to be watching for answers. Its WORKER COUNT, though, reads
  // its OWN stamp, `workersAt` — stamped only when the incoming entry carries `workers`, falling
  // back to `updatedAt` for an entry written before this change — because the monitor's own
  // keepalive (§8.3) can now move `updatedAt` with no fresh worker count behind it, and coupling
  // the count to that stamp would freeze a `-9`-killed conductor's count alive for as long as its
  // browser tab stayed open. An undercounted project would make every OTHER project launch MORE,
  // which is the wrong direction for an advisory cap — this file's header calls it "too few
  // launches, never too many". `beatAt` and `conductorSession` are still read by nothing in this
  // phase: they are the content spec §8.2 declares for the registry's entry, filled here, and
  // displayed by phase 4's `orchestra instances` — not dead fields, just not yet read.
