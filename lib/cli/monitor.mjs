...
// page keeps itself listed the way every other writer does: by reporting itself.
//
// Five minutes is twelve rewrites an hour of one small JSON file, and it is 72 times inside
// `lib/machine.mjs`'s six-hour staleness window — so a page stays listed across any single missed
// write, and the cost stays far below `orchestra ready`'s own, which rewrites the same file on
// every tick.
...
// false` — not installed, common on Linux, or killed by its own timeout), nothing is known, so
// nothing is refused and the bind decides: `monitorCommand` falls through to `serve(cfg)`, which
// finds the candidate port already held by the live page and probes upward to the next free one —
// so a second `orchestra monitor` on such a machine does not point at the first page, it opens a
// SECOND one on candidate+1 and records that port instead. The recorded port then flaps between
// the two numbers on every alternating run, which is the exact harm the one-page-per-project rule
...
export async function monitorCommand({ cfg, args }) {
  ...
  // `monitor.port` in the project's config, and a flag would be a second one — the first thing it
  // would break is the registry's promise that the recorded port is where the page is. Anything
  // else is refused rather than ignored, and refused HERE rather than after the refuse-and-point
  // below, or `orchestra monitor --prot` in a project whose page is already up would print "already
  // open", exit 0, and never mention the argument it did not understand.
  ...
  const { port, url } = await serve(cfg);
  ...
  out(`  ${url}`);
  out(`  ${cfg.root}`);
  // Last, so the URL is on screen before a browser is asked for — and so the registry already names
  // the port the browser is about to reach.
...
export function instancesCommand() {
  ...
  for (const e of live) {
    ...
    out(`${pad(e.name, 16)} ${pad(e.id, 7)} ${pad(e.mode, 8)} ${pad(url, 23)} ${pad(state(e), 12)} ${pad(workerWords(e), 15)} ${pad(sessionWords(e), 10)} ${pad(beatWords(e.beatAt, now), 10)} ${pad(sinceWords(e.updatedAt, now), 15)} ${e.root}`);
  }
}
