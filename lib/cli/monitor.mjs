...
//
// This file owns what the page needs that is not ANSWERING a request: the keepalive, the
// refuse-and-point, and the browser launch. It no longer owns the machine registry write alone —
// `lib/monitor/server.mjs`'s handler now shares it, calling `discoverProjects` on every request,
// which registers the current directory's project the first time it is unregistered — but that
// write is a locked, MERGING one (`recordInstance`, `lib/machine.mjs`), not a replace, and it is
// the only thing server.mjs touches under `~/.orchestra`: it still opens no browser window and
// runs no keepalive, so a test can build a handler, or bind a server, with nothing appearing on
// the developer's screen and no timer outliving the test.
