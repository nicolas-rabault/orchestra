import { resumeLimit } from '../register/workerPolicy.mjs';
...
import { codexOutbound, codexStatus } from '../register/codex.mjs';
...
export function driveCommand({ cfg, args }) {
  ...
  const obl = obligations(tasks, liveness, { maxAutoResumes: cfg.maxAutoResumes });
  const byId = new Map(tasks.map((t) => [t.id, t]));
  if (args.includes('--renew')) {
    const reason = flag(args, 'reason');
    if (ids.length !== 1 || !byId.has(ids[0]) || !reason?.trim())
      throw new Error('usage: drive --renew <key> --reason="specific remaining work"');
    if (obl.driving.some(t => t.id === ids[0])) throw new Error('cannot renew a running worker');
    const row = byId.get(ids[0]);
    if (TERMINAL.has(row.status)) throw new Error('cannot renew a terminal task');
    journal(cfg.root, { kind: 'note', task: ids[0], text: `Resume budget renewed: ${reason.trim()}` });
    writeState(cfg.root, { ...state, tasks: state.tasks.map(t => t.id === ids[0] ? { ...t, autoResumes: 0 } : t) });
    out(`renewed: ${ids[0]} — ${reason.trim()}; run drive separately`);
    return;
  }
  for (const row of obl.limited ?? []) out(`LIMIT: ${row.id} — resume budget reached; inspect the report, split or narrow the task before renewing`);
  for (const e of obl.refresh ?? []) out(`REFRESH: ${e.id} — ${e.why}`);
  ...
  for (const row of wanted) {
    ...
    if (!relayOwed(row) && resumeLimit(row, cfg.maxAutoResumes)) { out(`LIMIT: ${row.id} — resume budget reached`); continue; }
    if (!row.session) { out(`skip: ${row.id} — no session on the row; launch it (step 8)${relayOwed(row) ? ', with the relay in its brief' : ''}`); continue; }
    if ((row.runtime ?? 'claude') !== 'claude') {
      if (row.runtime !== 'codex') throw new Error(`unsupported worker runtime: ${row.runtime}`);
      if (codexStatus(row) === 'unknown') out(`REFRESH: ${row.id} — use wait_threads, then orchestra codex observe; no message dispatched`);
      else {
        try { out(JSON.stringify(codexOutbound(row, Date.now(), cfg))); nativeDispatches++; }
        catch (error) { out(`REFRESH: ${row.id} — ${error.message}; no message dispatched`); }
      }
      continue;
    }
    targets.push(row);
  ...
  for (const row of targets) {
    ...
    const p = startTurn(cfg, {
      row, cwd, slug, prompt: nudgeFor(row),
  ...
  if (!started.length) {
    if (nativeDispatches) { out(`${nativeDispatches} native outbound message(s) prepared; use send_message_to_thread then codex dispatched`); return; }
...
// One turn, started and left running. Shared with `orchestra retire`, whose wrap-up IS a worker
// turn and must be stopped, recorded, forked and logged in exactly this order — a second copy of
// this sequence beside it would be a second thing to keep right about how a worker is resumed.
//
// `stopFirst` is not optional in practice and is a parameter only so a caller that has already
// stopped the session does not stop it twice: a REGISTERED background session refuses `--resume`
// until it is stopped, and stopping it keeps its conversation.
export function startTurn(cfg, { row, cwd, slug, prompt, relay = null, registered = null, extra = {} }) {
  if ((row.runtime ?? 'claude') !== 'claude') throw new Error('native Codex tasks must use send_message_to_thread');
  const state = readState(cfg.root);
  const current = state?.tasks?.find(t => t.id === row.id);
  if (!current || current.session !== row.session) throw new Error('worker session changed before dispatch');
  if (!relay && !extra.retire && resumeLimit(current, cfg.maxAutoResumes)) throw new Error('resume budget reached');
  // Reserve before spawning: a failed start must not create an unbounded retry loop.
  if (!relay && !extra.retire)
    writeState(cfg.root, { ...state, tasks: state.tasks.map(t => t.id === row.id ? { ...t, autoResumes: (t.autoResumes ?? 0) + 1 } : t) });
  if (registered?.id) {
  ...
  writeTurn(p.record, {
    id: row.id, runtime: 'claude', session: row.session, cwd, pid: null, startedAt: new Date().toISOString(),
    endedAt: null, exit: null, refused: null, delivered: null,
    relay, prompt, log: p.log, ...extra,
  });
...
function runTurn(cfg, recordFile) {
  ...
  const row = readState(cfg.root)?.tasks?.find((r) => r.id === turn.id);
  if ((turn.runtime ?? 'claude') !== 'claude' || !row || (row.runtime ?? 'claude') !== 'claude' || row.session !== turn.session) throw new Error('turn runtime or session no longer matches row');
  ...
  // An absent recorded choice means inherit the resumed session, not today's project defaults.
  const selected = [...(row.model ? ['--model', row.model] : []), ...(row.thinking ? ['--effort', row.thinking] : [])];
  const child = spawn('claude', ['-p', '--resume', turn.session, ...selected, '--dangerously-skip-permissions', turn.prompt],
...
function stampReceipt(root, turn, at) {
  ...
  if (!row?.relay || (row.runtime ?? 'claude') !== 'claude' || row.session !== turn.session || row.relay.text !== turn.relay.text || (row.relay.writtenAt ?? null) !== (turn.relay.writtenAt ?? null)) return false;
