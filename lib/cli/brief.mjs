...
export function briefCommand({ cfg, args }) {
  ...
  process.stdout.write(renderBrief(cfg, row, {
    ...
    repo: repoArg,
    model,
    runtime: args.includes('--runtime') ? args[args.indexOf('--runtime') + 1] : workerRuntime({ ...row, ...registered }, { roadmapRuntimes: cfg.roadmapRuntimes, conductorRuntime: state?.conductor?.runtime ?? 'claude' }),
  }));
}
