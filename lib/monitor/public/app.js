...
import { slotOf, RUN_KEY, dropClosed, openItems, openQuestions, openRunAsks } from '/answers.mjs';
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      for (const m of state.model.projects) {
        // The run-level asks are passed too: their slots hang on no node, so a cache pruned
        // against `nodes` alone would forget the answer this tab had just sent to one — re-enabling
        // the box under its own "sent" line.
        dropClosed(cacheFor(answered, m.project.id), m.nodes, m.runAsks);
        dropClosed(cacheFor(drafts, m.project.id), m.nodes, m.runAsks);
      }
      render();
      refreshCard();
    }
...
function render() {
  ...
  // A run-level question on screen is proof the register exists, so this sentence would be a
  // contradiction printed beside it — which is the exact state the end of a run leaves: no node,
  // and a question waiting.
  $('empty').hidden = view.tab !== LOCAL || m.nodes.length > 0 || (m.runAsks ?? []).length > 0;
  ...
  const openRun = openRunAsks(m.runAsks, sent);
  renderRunAsks(m.runAsks ?? [], openRun);
  // The corner and the strip count both kinds. A question about the run is waiting on the user
  // exactly as a question about a row is, and the one place it must never be missing is the surface
  // that says how many things are waiting.
  renderNotifs([...openNow, ...openRun.map(runNotif)]);
  renderTally(tab.counts, [...openNow, ...openRun], sent);
}
...
function renderTally(c, open, sent) {
  ...
  pill('active', c.active, 'in flight', 'unfinished tasks (claimed or in review), not a count of running sessions');
...
// ---- the question about the run itself ----
// One frame docked over the canvas, holding the asks that belong to no row. It is the surface the
// end-of-run question needed and did not have: a run ENDS when its last row goes terminal, the
// canvas is empty by then, and the question the protocol makes obligatory had nowhere to be drawn
// (ticket `t-0antbtb` — "je ne vois pas de question sur la page", and the page was right).
//
// Rebuilt only when what it says changes, exactly like the card and for the same reason: a poll runs
// every two seconds, and a panel that replaced its own textarea on every one of them would take the
// caret out of the answer being typed into it. What the user typed and what they have already sent
// both live outside the DOM, by slot, so neither is lost by a rebuild.
let runStamp = null;

function renderRunAsks(items, open) {
  const panel = $('runasks');
  panel.hidden = items.length === 0;
  if (!items.length) { runStamp = null; return; }
  // The same two colours the ask blocks and the nodes use, on the frame itself: alarm while
  // something is waiting on the user, teal once it is answered and only orchestra is late. A panel
  // that stayed alarm-red over a block that had gone teal is the page contradicting itself about the
  // one thing it is being asked.
  panel.classList.toggle('answered', open.length === 0);
  $('runasks-head').textContent = open.length
    ? `${open.length === 1 ? 'a question' : `${open.length} questions`} about this run`
    : 'answered — waiting for orchestra';
  const sentRun = [...cacheFor(answered, current().project.id).keys()].filter((k) => k.startsWith(`${RUN_KEY}:`));
  const stamp = JSON.stringify([items, sentRun]);
  if (stamp === runStamp || sending) return;
  runStamp = stamp;
  $('runasks-body').replaceChildren(...items.map((a) => askBlock(runTarget(), a)));
}

// Reached from the corner list, which lists a run-level question beside the row ones: there is no
// card to open, so the panel is scrolled to and the caret put in its box — the same act, one surface
// over.
function focusRunAsk(id) {
  const blocks = [...$('runasks-body').children];
  const i = (current().runAsks ?? []).findIndex((a) => a.id === id);
  const block = i >= 0 ? blocks[i] : null;
  if (!block) return;
  block.scrollIntoView({ block: 'nearest' });
  block.querySelector('textarea')?.focus();
}

// A run-level ask as one row of that list: the run it is about where a task id would be, because
// that is what it is about. Listed beside the row questions rather than in a list of its own — the
// corner answers one question, "what is waiting on you", and two lists would answer it twice.
const runNotif = (a) => ({
  run: true, key: RUN_KEY, id: a.roadmap ? `run · ${a.roadmap}` : 'this run',
  // `question` alongside `ask` for the same reason `openQuestions` carries it: the row is cut to
  // ninety characters, and a run-level ask is shaped by the same template, so the body's opening is
  // its context and never the thing being asked.
  item: a.id, kind: a.kind, ask: a.ask, question: a.question?.text ?? null,
  images: (a.images ?? []).length,
});

function renderNotifs(rows) {
  $('notifs').hidden = rows.length === 0;
  $('notifs-head').textContent = `${rows.length} waiting on you`;
  ...
  for (const q of rows) {
    ...
    // The question and not the body: the row is cut to ninety characters, so every ask the decision
    // template shapes used to show the opening of its CONTEXT — "Where it stands: the 3D picture of
    // the arm slowly falls behind the real arm during a lo…" — and never the thing being asked. The
    // body is the fallback, for an ask that puts no question of its own.
    const text = q.question ?? q.ask ?? '';
    const short = excerpt(text, NOTIF_EXCERPT_CAP);
    li.append(head, el('div', 'what', short ? `${short}…` : (text || '(no text)')));
    li.onclick = () => (q.run ? focusRunAsk(q.item) : select(q.key, q.item));
    list.append(li);
  }
}
...
const ANSWER_WORDS = {
  queued: 'answer saved; wake queued for the existing conductor',
...
// A row's question: posted against that row, remembered under its node key, and it redraws the
// card it is drawn in — keeping its scroll, because the card is what the reader is looking at.
const nodeTarget = (node) => ({
  key: node.key,
  task: node.id,
  refresh: () => { const top = card.scrollTop; openCard(node); card.scrollTop = top; },
});

// A question about the run: posted against no task, remembered under the run's own namespace, and
// redrawn by `render()` alone — which the caller has already called, so there is nothing to do here.
const runTarget = () => ({ key: RUN_KEY, task: null, refresh: null });

// `target` is what the answer is about, in the three terms this block needs and nothing more: the
// slot namespace it remembers a sent answer under, the task id the answer is posted against (null
// for a question about the run, which is about no task), and how to redraw the surface it is drawn
// on. A run-level ask is the same question in a different frame, so it is the same code.
function askBlock(target, item) {
  const slot = slotOf(target.key, item.id);
  ...
  box.append(el('div', 'kind', item.kind + (item.port ? ` · port ${item.port}` : '')));
  // The context first, each of the template's labels drawn as the heading it is. The body used to
  // be printed as one paragraph, so a labelled context, the question and a paragraph of
  // justification arrived as one undifferentiated block and the reader had to find the question in
  // it. An ask carrying no label at all is one section with no heading, which is what it was.
  for (const s of item.context) {
    const sec = el('div', 'section');
    if (s.label) sec.append(el('div', 'label', s.label));
    sec.append(el('p', null, s.text));
    box.append(sec);
  }
  if (!item.ask) box.append(el('p', null, '(no text)'));
  // Between the context and the question, which is the order the question is read in: a "which of
  // these two?" is unanswerable until the two are on screen.
  const asked = shots(item.images);
  if (asked) box.append(asked);
  // And the question last, whatever order it arrived in — the template puts `Why it is yours to
  // decide` AFTER it, and a reader who has to hold the question in mind across a paragraph of
  // justification reads it twice. Here it sits directly above the buttons that answer it.
  if (item.question) box.append(el('div', 'q', item.question.text));
  ...
  async function submitOnce(answer) {
    ...
    setDisabled(true);
    const line = await postAnswer({ task: target.task, pending: item.id, answer }, status);
    ...
    // answer.
    render();
    // And the surface the user is actually looking at. Nothing else rebuilds it: `refreshCard`
    // compares the node it was drawn from, and that node does not change until a tick clears the
    // item — so the block stayed alarm-red underneath its own "sent" line. Its scroll is kept, and
    // the sent line and the disabled controls are rebuilt from `answered`, not from the DOM.
    target.refresh?.();
  }
...
// What this task can be OPENED in, as one button per entry: its own dev server, its own pull
// request, and whatever else orchestra wrote on the row — one list, built server-side
// (`openablesFor`, lib/monitor/model.mjs). One renderer rather than one per kind, because they are
// one gesture: a link orchestra wrote and a port lsof found must not drift apart in wording.
//
// Each button says in words what it opens, and names the host it leaves for underneath. The label
// and the URL are written by the same agent, so the label alone promises nothing — the host is the
// half the reader can check before clicking.
//
// The whole-machine list — most of it irrelevant to this task — folds behind a single expandable
// line naming how many others there are. A task with nothing to open is a normal, common state,
// said plainly rather than left to read as a missing or broken list.
function whatToOpen(node) {
  const own = el('div', 'own');
  for (const o of node.openables) {
    const row = el('div', 'own-entry');
    const a = el('a', 'own-link', `open ${o.label} →`);
    a.href = o.url;
    a.target = '_blank';
    // These URLs come out of the register, which an agent writes. `noopener` so a page opened from
    // here cannot reach back through `window.opener` and navigate the board out from under the user.
    a.rel = 'noopener noreferrer';
    row.append(a, el('span', 'muted', o.where));
    own.append(row);
  }
  // "Nothing to open" and "I could not look" are different answers, and only one of them is a fact:
  // lsof exits non-zero both when nothing is listening and when it is not installed at all. Either
  // is said UNDER whatever buttons there are and never instead of them — whether a port could be
  // probed has nothing to do with whether this task has a pull request, and the row that most needs
  // its button is the one with no worktree and no server at all.
  if (!current().serversKnown) {
    own.append(el('p', 'muted', 'no port could be checked here — lsof could not be run on this machine, so a dev server would not show up'));
  } else if (!node.openables.length) {
    own.append(el('p', 'muted', 'nothing to open for this task yet — no dev server running, and no link recorded'));
  }
  ...
  if (others.length) {
    ...
    details.append(el('summary', null, `${others.length} other dev server${others.length === 1 ? '' : 's'} on this machine — ${others.length === 1 ? 'it is not' : 'none of them is'} this task's`), list);
    children.push(details);
  }
  return section('what to open', ...children);
}
...
function openCard(node, focus = null) {
  ...
  if (node.pending.length) {
    body.append(section(openItems(node, sentSlots()).length ? 'waiting on you' : 'answered — waiting for orchestra',
      ...node.pending.map((p) => {
        const block = askBlock(nodeTarget(node), p);
  ...
  if (node.invoke.kind !== 'none') {
    body.append(section(node.runtime === 'codex' && node.invoke.kind === 'resume' ? 'prepare its message; send it with send_message_to_thread' : node.invoke.kind === 'resume' ? 'wake its session' : 'launch it', cmdBlock(node.invoke.lines)));
  }

  body.append(whatToOpen(node));
  ...
  row('runtime', `${node.runtime ?? 'claude'} · ${node.workerStatus ?? 'unknown'}`);
  row('session', node.runtime === 'codex' ? node.session : node.sessionName ? `${node.sessionName} (${(node.session ?? '').slice(0, 8)}) on ${node.model ?? '?'}` : '—');
