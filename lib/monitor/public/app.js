...
// The list of things this task can be opened in: its own pull request when it has one, then its own
// dev server (the model's per-node `servers`, matched by port), both drawn as the same button
// because they are the same gesture. The whole-machine list — most of it irrelevant to this task —
// folds behind a single expandable line naming how many others there are. A task with no server of
// its own is a normal, common state, said plainly rather than left to read as a missing/broken list.
function serversSection(node) {
  const own = el('div', 'own');
  // A pull-request review row leads with its own PR. `prUrl` is built server-side
  // (`pullRequestUrl`, lib/monitor/model.mjs) and is null on every other row and in every project
  // whose origin is not on GitHub, so this draws nothing unless there is really somewhere to go.
  // It sits ABOVE the lsof branch below on purpose: whether a port could be probed has nothing to
  // do with whether this task has a pull request, and the row that most needs this button is the
  // one with no worktree and no server at all.
  if (node.prUrl) {
    const row = el('div', 'own-entry');
    // By construction the last segment of the URL is the number — `pullRequestUrl` builds it.
    const a = el('a', 'own-link', `open PR #${node.prUrl.split('/').pop()} on GitHub →`);
    a.href = node.prUrl;
    a.target = '_blank';
    row.append(a);
    own.append(row);
  ...
  if (!current().serversKnown) {
    ...
    return section('active dev servers', own);
  }
  ...
  return section('active dev servers', ...children);
}
