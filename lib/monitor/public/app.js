...
const SHOT_PX = 44;
const imageUrl = (im) => `/api/image?p=${encodeURIComponent(im.rel)}`;
...
const loadPlaced = () => {
  try {
    ...
    // Filtered on read, not trusted: this is the only input to the layout that a user (or a stale
    // format) can hand it, and a NaN coordinate would place a frame nowhere at all.
...
const savePlaced = () => {
  try { localStorage.setItem(PLACED_KEY, JSON.stringify(Object.fromEntries(state.placed))); }
  catch { /* no storage (private window, quota): the arrangement holds for this session only */ }
};

const state = { model: null, etag: null, collapsed: new Set(), selected: null, filter: '',
  view: { x: 24, y: 24, k: 1 }, placed: loadPlaced(), tab: LOCAL };
...
const visible = () => nodesOf(state.model.nodes, state.tab);
const graph = () => layout(visible(), { collapsed: state.collapsed, placed: state.placed });
...
// Which project this tab is piloting. The server already wrote the name into the served HTML, so
// this is not what puts it on screen first — it is what keeps it right: a project renamed in its
// config is a new name on the next poll, with no reload. Written to BOTH the tab and the header,
// because a person with four of these open reads the tab strip long before they read the page.
...
async function poll() {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      nameProject(state.model.project);
      // Before anything is drawn: forget what the user typed and sent for items orchestra has
      // closed. A pending id with no id of its own is a hash of row/kind/ask, so the same question
      // asked next round carries the same key — and this tab may have been open for hours.
      dropClosed(answered, state.model.nodes);
      dropClosed(drafts, state.model.nodes);
      render();
      refreshCard();
    }
...
function drawGraph() {
  ...
  for (const e of g.edges) {
    ...
    p.setAttribute('class', [e.cross ? 'cross' : '', (state.selected === e.from || state.selected === e.to) ? 'lit' : ''].filter(Boolean).join(' '));
    svg.append(p);
  }
  ...
  for (const n of shown) {
    ...
    const node = el('div', `node ${n.status}${calm ? ' answered' : ''}${waiting ? ' attention' : ''}${state.selected === n.key ? ' selected' : ''}`);
...
function render() {
  ...
  if (!tabs.some((t) => t.id === state.tab)) state.tab = LOCAL;
  ...
  $('empty').hidden = state.tab !== LOCAL || m.nodes.length > 0;
  ...
  $('tidy').hidden = state.placed.size === 0;
...
function dragFrame(f, e) {
  ...
  const move = (ev) => {
    ...
    moved = true;
    state.placed.set(f.roadmap, {
      x: Math.max(0, from.x + (ev.clientX - at.x) / state.view.k),
      y: Math.max(0, from.y + (ev.clientY - at.y) / state.view.k),
    });
    drawGraph();
  };
  const up = () => {
    ...
    if (!moved) {
      state.collapsed.has(f.roadmap) ? state.collapsed.delete(f.roadmap) : state.collapsed.add(f.roadmap);
      return render();
    }
    savePlaced();
...
function renderTally(c, open, sent) {
  ...
  const held = state.model.nodes.reduce((n, node) => n + node.pending.length - openItems(node, sent).length, 0);
...
function renderTabs(tabs) {
  ...
  host.replaceChildren();
  for (const t of tabs) {
    const b = el('button', t.id === state.tab ? 'on' : null, t.label);
    ...
    b.onclick = () => { if (state.tab !== t.id) { state.tab = t.id; render(); } };
    host.append(b);
  }
}
...
function select(key, focusItem = null) {
  ...
  const node = state.model.nodes.find((n) => n.key === key);
  ...
  state.view.k = Math.max(state.view.k, MIN_LEGIBLE_K);
  ...
  state.view.x = clampAxis(naiveX, g.width, box.width, k);
  state.view.y = clampAxis(naiveY, g.height, box.height, k);
  applyView();
}
...
card.addEventListener('close', () => { state.selected = null; render(); });
...
const sentSlots = () => new Set(answered.keys());
...
function askBlock(node, item) {
  ...
  const box = el('div', `ask${item.answer || answered.has(slot) ? ' answered' : ''}`);
  ...
  text.value = drafts.get(slot) ?? '';
  text.oninput = () => drafts.set(slot, text.value);
...
function serversSection(node) {
  ...
  const others = state.model.servers.filter((s) => !node.servers.some((x) => x.port === s.port));
  const children = [own];
  if (others.length) {
    const list = el('ul');
...
const cardSignature = (node) => JSON.stringify([
  node, state.model.rail.filter((r) => r.task === node.key), state.model.servers, state.model.serversKnown,
]);
let cardStamp = null;
...
function refreshCard() {
  if (!card.open || !state.selected || sending) return;
  const node = state.model.nodes.find((n) => n.key === state.selected);
  if (!node) return card.close();                       // the task left orchestra's records
...
function openCard(node, focus = null) {
  ...
  const lines = state.model.rail.filter((r) => r.task === node.key);
  const rail = el('div');
  for (const r of lines) {
    const row = el('div');
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  drag = { x: e.clientX - state.view.x, y: e.clientY - state.view.y };
...
canvas.addEventListener('wheel', (e) => {
  ...
  const k = Math.min(2, Math.max(0.2, state.view.k * (e.deltaY < 0 ? 1.1 : 1 / 1.1)));
...
const zoomBy = (r) => { state.view.k = Math.min(2, Math.max(0.2, state.view.k * r)); applyView(); };
$('zoom-in').onclick = () => zoomBy(1.15);
$('zoom-out').onclick = () => zoomBy(1 / 1.15);
$('fit').onclick = fit;
// Back to the automatic flow, in one click, for every frame at once. Nothing else undoes a drag, and
// an arrangement you cannot get out of is a trap rather than a preference.
$('tidy').onclick = () => { state.placed.clear(); savePlaced(); render(); fit(); };
$('rail-filter').oninput = (e) => { state.filter = e.target.value; renderRail(); };
...
applyView();
await poll();
// The first poll can fail (the server was killed between the page load and the fetch); fitting a
// model that is not there would throw and leave a blank page with no explanation.
if (state.model) fit();
