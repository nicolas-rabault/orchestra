...
const views = new Map();
const viewFor = (id) => {
  if (!views.has(id)) views.set(id, { collapsed: new Set(), selected: null, view: { x: 24, y: 24, k: 1 }, tab: LOCAL, fitted: false });
  return views.get(id);
};
...
// How many of the dialog's own `close` events are OURS — triggered by leaving a project, rather
// than by the reader pressing Escape or the × button. `close()` removes the `open` attribute the
// instant it is called, but the `close` EVENT it fires is a queued task, not a synchronous
// callback and not even a microtask — measured against a live `<dialog>`, it can land tens of
// milliseconds later. By the time it does, `current()` already names the project just switched TO,
// not the one whose card this actually was: left to the listener below alone, it would null the
// WRONG project's `selected`, silently decoupling a card freshly opened in the new project from the
// poll-driven refresh that is supposed to keep it live. A count, not a boolean, because two
// switches close together — each closing a genuinely open card — must each consume their own event
// rather than the first swallowing both.
let pendingSwitchCloses = 0;
...
function leaveProject(id) {
  if (card.open) { pendingSwitchCloses++; card.close(); }
  if (id) viewFor(id).selected = null;
}

card.addEventListener('close', () => {
  // A close WE triggered while leaving a project already had its `selected` cleared, synchronously,
  // by `leaveProject` — by the time this queued event fires, `current()` may no longer even be the
  // project being left, so it must not act a second time.
  if (pendingSwitchCloses > 0) { pendingSwitchCloses--; return; }
  const here = current();
