...
// IT CREATES NO CONDUCTOR. An enabled Codex transport queues a wake to the existing session
// after the inbox append; it never starts a competing tick session.
//
...
import {
  SERVERS_TTL_MS, currentBranch, originRepo, imageFinder, listServers, readBoard, readInbox, readJournal,
...
export function createHandler({ projects, port, publicDir, notify = notifyConductor }) {
  ...
  const modelFor = ({ cfg }, state, found) => {
    ...
    return buildModel({
      project: {
        name: cfg.name, root: cfg.root, mode: cfg.mode, branch: currentBranch(cfg.root),
        // `owner/name` when this checkout's origin is on GitHub, else null. The model turns it
        // into a pull-request row's link out; nothing else reads it.
        repo: originRepo(cfg.root), id: cfg.id,
      },
      cfg,
      board: readBoard(cfg.root),
      register,
      // The register's run-level questions, which hang on no row and reach the page as their own
      // panel — the one kind of ask that exists precisely when every row is terminal.
      runAsks: state.runAsks ?? [], runAnswered: state.runAnswered ?? [],
      ...
      findImage: imageFinder(cfg.root, worktreePaths(cfg.root)),
    });
  };

  // `set` is the ONE resolution of `projects()` for this request — see the route handlers below,
  // which each call `projects()` exactly once and hand the result to both `model` and
  // `projectById`. `discoverProjects` is not cheap (its own header states the cost: two `git`
  // forks per known root), so calling it twice per request — once for the etag, once for the body,
  // as this used to — doubled that cost for nothing, and could let the etag describe a DIFFERENT
  // set than the one the body actually serves if a project joined or left between the two calls.
  const model = (set) => {
    ...
    //
    // No sort here: `set`'s own order is preserved verbatim. `discoverProjects` already sorts by
    // name then id, and the strip the user sees is sorted a second time client-side by
    // `projectTabsOf` — a third sort here would be a third definition of the same rule with no
    // user-visible effect, since nothing reads `model.projects` positionally (`modelOf` looks up by
    // id).
    const rows = set.map((p) => ({ p, state: readState(p.cfg.root) }));
    const found = devServers(rows.flatMap(({ state }) => portsOf(state.tasks ?? [])));
    return { machine: { port }, projects: rows.map(({ p, state }) => modelFor(p, state, found)) };
  };
  ...
  const projectById = (id, set) => (typeof id === 'string' && id ? set.find((p) => p.id === id) ?? null : null);
  ...
  return async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/model') {
        // Resolved ONCE for this request: the etag and the body must describe the SAME set, never
        // two separate calls to `projects()` that a project joining or leaving between them could
        // make disagree.
        const set = projects();
        ...
        const etag = `"${Buffer.from(set.map((p) => sourceStamp(p.cfg)).join('|')).toString('base64url')}"`;
        if (req.headers['if-none-match'] === etag) return send(res, 304, '');
        return json(res, 200, model(set), { etag, 'cache-control': 'no-cache' });
      }
      ...
      if (req.method === 'GET' && url.pathname === '/api/image') {
        const owner = projectById(url.searchParams.get('project'), projects());
      ...
      // The answer channel's one writer. It reaches a conductor; IT NEVER CREATES ONE.
       // Enabled Codex queue delivery wakes that same UUID after the durable append.
      //
      ...
      if (req.method === 'POST' && url.pathname === '/api/answer') {
        ...
        const owner = projectById(body.project, projects());
        ...
        const wake = notify(owner.root, 'monitor answer available');
        return json(res, 200, { ok: true, conductor: wake === 'queued' ? 'queued' : liveConductor(owner.root) ? 'awake' : 'no-conductor' });
      }
...
// Binds, and hands back the server it bound with the port it actually got. It starts no process —
// the refuse-and-point, the keepalive and the browser launch still all belong to
// `lib/cli/monitor.mjs` — but it is no longer a stranger to `~/.orchestra`: `machineMonitorPort()`
// below reads `~/.orchestra/machine.json` for the bind candidate, and the handler it wires up calls
// `discoverProjects` on every request, which can WRITE `~/.orchestra/instances.json` (through
// `recordInstance`) the first time it finds the current directory's project unregistered — spec
// §2's "recorded on the way". What still holds, narrower than the old claim: this module opens no
// browser window and runs no keepalive, so a test can build a handler, or bind a server, with
// nothing appearing on the developer's screen and no timer outliving the test.
//
...
export async function serve({ cwd = process.cwd() } = {}) {
  ...
  const bound = await listenOnFreePort(server, { port: machineMonitorPort() });
