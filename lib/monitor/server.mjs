...
// page's assets and has nothing to say about them.
const SHARED_MODULES = ['/layout.mjs', '/answers.mjs', '/clock.mjs', '/progress.mjs', '/tabs.mjs'];
...
export function createHandler({ cfg, port, publicDir }) {
  ...
  return async (req, res) => {
    try {
      ...
      //
      // The page this was ported from spawned `cron-tick.sh` when no conductor was beating. There
      // is no such script here — the tick runner is a later phase's — and a page that started a
      // bare `claude -p` instead would re-commit the failure that source's own comment records:
      ...
      if (req.method === 'POST' && url.pathname === '/api/answer') {
        let body;
        try { body = JSON.parse(await readBody(req)); } catch { return json(res, 400, { ok: false, error: 'body is not JSON' }); }
...
// is not a reason to fail: `orchestra monitor` prints the URL either way and the page is there.
function openBrowser(url) {
  ...
  child.on('error', () => process.stdout.write(`could not open a browser — the page is at ${url}\n`));
  child.unref();
}

// Binds, and hands back the server it bound with the port it actually got. It touches
// `~/.orchestra` not at all: the machine registry, the refuse-and-point and the keepalive belong to
// `lib/cli/monitor.mjs`, which is what lets a test call `createHandler` in-process without going
// anywhere near the developer's own registry.
//
...
export async function serve(cfg, { open = true } = {}) {
