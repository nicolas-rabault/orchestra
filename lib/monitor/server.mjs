...
export function createHandler({ projects, port, publicDir }) {
  ...
  const model = () => {
    ...
    //
    // No sort here: `projects()`'s own order is preserved verbatim. `discoverProjects` already
    // sorts by name then id, and the strip the user sees is sorted a second time client-side by
    ...
    // id).
    const set = projects().map((p) => ({ p, state: readState(p.cfg.root) }));
