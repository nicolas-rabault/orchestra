...
export function projectTabsOf(models, sentFor = () => new Set()) {
  return (models ?? [])
    .map((m) => ({
      id: m.project.id,
      name: m.project.name,
      ...
      // `sentFor(id)` is a FUNCTION, not one Set: a slot key (`nodeKey:itemId`) is only unique
      // WITHIN a project, so two projects can carry the identical key for two unrelated questions.
      // Applying one flat Set to every project's `openQuestions` call would let an answer sent in
      // project A silently drop project B's identical-keyed question off its own badge — which is
      // exactly the collision this whole strip exists to keep apart. Called once per model, so
      // each project's badge is filtered only by what THIS tab has sent for THAT project.
      waiting: openQuestions(m.nodes, sentFor(m.project.id)).length,
    }))
