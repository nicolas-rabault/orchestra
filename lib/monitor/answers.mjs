...
export function openQuestions(nodes, sent = new Set()) {
  ...
  for (const n of nodes ?? []) {
    for (const p of openItems(n, sent)) {
      // `question` beside `ask`, because the row is excerpted to ninety characters and the body
      // opens on the context: every ask the decision template shapes showed the reader "Where it
      // stands: …" and never the thing being asked. The body stays, as the fallback for an ask
      // that puts no question of its own.
      out.push({
        key: n.key, id: n.id, item: p.id, kind: p.kind,
        ask: p.ask, question: p.question?.text ?? null, images: (p.images ?? []).length,
      });
    }
  }
  return out;
