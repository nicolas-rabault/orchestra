...
// the browser (served at the route Task 4 serves) and imported by the tests, the same way
// inbox.mjs is shared by every reader of the answer channel — `lib/register/{tick,relay,watch}.mjs`
// today, and the `orchestra-inbox` hook P5 adds: the rule that decides what the page remembers is
// written once.
//
...
export function openSlots(nodes) {
  const open = new Set();
...
export function dropClosed(cache, nodes) {
  const open = openSlots(nodes);
