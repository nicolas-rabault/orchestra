...
export function invokeCommand(node, cfg) {
  ...
  return { kind: 'launch', lines: [
    ...
    `claude --bg -n ${session} --model ${workerSelection(cfg, node, 'claude').model} --effort ${workerSelection(cfg, node, 'claude').thinking} --dangerously-skip-permissions "<brief>"`,
  ] };
}
...
export function buildModel({
  ...
  const joined = pairs.map(({ key, board: b, reg }) => {
    ...
    const node = {
      ...
      // Preserve explicit review states, but an enrolment placeholder must not hide git progress.
      status: reg?.status && reg.status !== 'todo' ? reg.status : b?.status ?? reg?.status ?? 'todo',
