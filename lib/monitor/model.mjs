...
export function invokeCommand(node, worktrees, cfg) {
  ...
  // imported here to own the choice in one place: it reaches `node:child_process` (via
  // `gatherGit`), and these six modules are served to the browser as plain ES modules
  // (`progress.mjs`'s own header), where a `node:` import cannot resolve at all. Left duplicated
  // on purpose; if the two ever disagree, `ready.mjs` is right.
...
export function buildModel({
  ...
  // conductor has taken it — and the route Task 4 serves, and the hook P5 adds, will too: one
  // rule, every reader.
  //
