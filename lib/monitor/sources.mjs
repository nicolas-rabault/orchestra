...
// How long a dev-server list stays good, and the clock bucket `sourceStamp`'s etag expires on
// (below). Was also the board's own success-cache TTL until the 2026-09-04 branch review split
// them — see `BOARD_SUCCESS_TTL_MS`, further down, for why 5 s was too fast a cadence for a `gh`
// round trip to repeat on. Fusing the two was faithful to only one requirement, which still
// holds after the split: the etag must expire AT LEAST as fast as the board's own cache, never
// that the two numbers must be equal — a faster etag just means an occasional rebuild that reads
// the still-cached board instead of a wasted network round trip.
...
// How long a SUCCESSFULLY-read board is served before it is read again — decoupled from
// SERVERS_TTL_MS (branch review, 2026-09-04): the two were one constant, and fusing them at 5 s
// meant a page tab, which polls every 2 s with no `document.hidden` gating, landed past the
// window on nearly every poll and forced a real board read. Offline that is ~12 node starts a
// minute for nothing; online each one is a `gh` round trip, measured at 3.0 s in this module's own
// header above (2026-08-13 in planetCraft, 8 worktrees) — a continuous ~60% duty cycle against
// GitHub for as long as a tab stays open, which the protocol now tells a conductor to do for hours
// at a time (eight, in planetCraft). 30 s cuts that duty cycle to near zero while staying far
// under BOARD_RETRY_MS, so a real change to the shared board is still visible well inside one
// failure-memory window of it happening.
export const BOARD_SUCCESS_TTL_MS = 30_000;
...
export function readBoard(root, { bin = DEFAULT_BIN, timeoutMs = BOARD_TIMEOUT_MS } = {}, now = Date.now()) {
  ...
  if (entry.lastGood && !entry.failure && now - entry.goodAt < BOARD_SUCCESS_TTL_MS) return entry.lastGood;
...
// The production bound for every child process in this module except `readBoard`'s own, longer,
// deadline (a healthy board read is itself slower than this). `git worktree list` and `lsof` are
// both ordinary and fast in the healthy case, but this server is single-threaded, and an unbounded
// child here blocks every other reader exactly the way the nine-hour 2026-08-12 GitHub incident
// (see `readBoard`, above) blocked the whole page — just without a network call to blame it on.
const CHILD_TIMEOUT_MS = 2000;

// `timeoutMs` is a parameter on `worktreePaths` and `listServers`, below, for the same reason it is
// one on `readBoard`: a test that proves the wait is bounded has to wait for it, and a test
// budgeted at the real 2s production value is a test nobody runs quickly. It is NOT a tuning knob —
// every real caller gets `CHILD_TIMEOUT_MS` by not passing anything, and nothing should ever pass a
// different one outside a test.
export function worktreePaths(root, timeoutMs = CHILD_TIMEOUT_MS) {
  const map = new Map();
  let path = null;
  try {
    ...
    const opts = { stdio: ['ignore', 'pipe', 'ignore'], env: gitEnv(), timeout: timeoutMs };
...
// `owner/name` for a checkout whose `origin` is on GitHub, and null for everything else. The one
// fact the page needs to turn a pull-request review row into a link, and it is READ rather than
// configured: this plugin is installed into projects it has never heard of.
//
// Read out of `.git/config` for the same reason `currentBranch` reads `.git/HEAD` — the page reads
// this synchronously on every request, and a reader that shells out is a reader that can hang.
// `readBoard`'s own header carries what that cost when GitHub went unreachable for nine hours.
//
// Only the MAIN checkout is asked, which is all `server.mjs` ever passes: a linked worktree keeps
// no `config` of its own, so following its `gitdir:` the way `currentBranch` must would buy nothing
// here.
//
// Null covers three different answers on purpose — no repository, no `origin`, an `origin` that is
// not GitHub — because the page does the same thing with all three: it draws no button. Inventing a
// URL shape for a host this plugin has never seen would produce one that 404s, which is worse.
export function originRepo(root) {
  try {
    const config = readFileSync(join(root, '.git', 'config'), 'utf8');
    // The section body runs to the next `[`, so `origin` is found by name rather than by being the
    // first remote in the file: a fork's `upstream` names a repository whose pull requests are not
    // the ones this board swept.
    const section = /\[remote "origin"\]([^[]*)/.exec(config);
    const url = section && /^\s*url\s*=\s*(.+?)\s*$/m.exec(section[1])?.[1];
    const m = url && /^(?:git@github\.com:|[a-z+]+:\/\/(?:[^@/]*@)?github\.com\/)([^/]+\/[^/]+?)(?:\.git)?\/?$/.exec(url);
    return m ? m[1] : null;
  } catch { return null; }
}
...
// `timeoutMs` defaults to `CHILD_TIMEOUT_MS` for the same reason `worktreePaths`' does, above: it
// exists so a test can prove the bound in milliseconds rather than in the real production wait, and
// it is not a tuning knob — nothing should ever call this with anything but the default.
export function listServers(ports = [], timeoutMs = CHILD_TIMEOUT_MS) {
  ...
  let out = '';
  // lsof is the canonical binary that hangs on a stale network mount, and this server is
  // single-threaded — an unbounded call here is the nine-hour 2026-08-12 GitHub incident (see
  // `readBoard`, above) reached through a different door. A timeout kills the child and lands in
  // the catch below exactly like every other lsof failure this reader already handles.
  try { out = run('lsof', ['-nP', `-iTCP:${unique.join(',')}`, '-sTCP:LISTEN', '-F', 'pn'], { timeout: timeoutMs }); }
...
export function sourceStamp(cfg, now = Date.now()) {
  ...
  // idle — and this module's own cache promises a few seconds. This same bucket also carries the
  // BOARD: online, a `gh` round trip is the only thing that changes it, and no local file's mtime
  // moves when GitHub does — without this, a reader chasing a stale board on screen would go
  // looking for something on disk to stat and find nothing to blame. A coarse bucket of the clock
  // expires the etag at this cadence and no faster; it is faster than the board's own success
  // cache (`BOARD_SUCCESS_TTL_MS`, above), which only costs an occasional rebuild that reads the
  // still-cached board rather than a wasted network round trip.
